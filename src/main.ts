...
const missingWfp = ['WAYFORPAY_MERCHANT_ACCOUNT', 'WAYFORPAY_SECRET_KEY', 'WAYFORPAY_MERCHANT_DOMAIN', 'WAYFORPAY_WEBHOOK_BASE_URL']
  .filter((key) => !process.env[key]);
if (missingWfp.length > 0) {
  logger.warn(`Missing WayForPay env vars: ${missingWfp.join(', ')} — card payments will not work`);
}

const missingUsdt = ['USDT_WALLET_ADDRESS', 'ME_ADMIN_CHANNEL_ID']
  .filter((key) => !process.env[key]);
