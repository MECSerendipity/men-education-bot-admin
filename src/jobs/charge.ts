  ...
  for (const sub of subs) {
    ...
    if (result.success) {
      ...
      try {
        ...
        await bot.telegram.sendMessage(
          sub.telegram_id,
          `\u{26A0}\u{FE0F} Не вдалося автоматично продовжити підписку.\n\n` +
          `Причина: ${result.reason ?? 'Помилка оплати'}\n` +
          `${urgency}\n\n` +
          `Оплатіть вручну, щоб зберегти доступ.`,
          {
            reply_markup: {
              inline_keyboard: [
                [{ text: '\u{1F4B3} Продовжити підписку', callback_data: 'subscription' }],
              ],
            },
          },
        );
